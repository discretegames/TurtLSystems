...
Try them by running this file and editing the comments at the bottom, or importing into your own file like:

from TurtLSystems.examples import *

Then call the various functions:
...
def koch_snowflake(level: int = 5, size: float = 300,  anti: bool = False,) -> None:
    ...
    print(draw(start, 'F F+F--F+F', level, 60, length, color=(0, 255, 255), asap=True,
...
def square_koch_curve(level: int = 5, width: float = 400) -> None:
    ...
    print(draw('F', 'F F+F-F-F+F', level, 90, length, color=(0, 0, 255), asap=True, position=(-width / 2, 0))[0])
...
def tree(level: int = 6, size: float = 200) -> None:
